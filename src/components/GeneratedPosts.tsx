...
import { PostIdea } from '../types/posts';
...
export default function GeneratedPosts() {
  ...
  const handleDeletePost = (post: PostIdea) => {
    setShowDeleteConfirm(post);
  };

  const confirmDelete = () => {
    if (showDeleteConfirm) {
      setGeneratedPosts(generatedPosts.filter(p => p !== showDeleteConfirm));
  ...
  const handleSaveConfiguration = async (configuredPost: ConfiguredPost) => {
    try {
      ...
      setApprovedPosts(prev => [...prev, formattedPost]);
      
      // Remove from generated posts
      setGeneratedPosts(prev => prev.filter(p => p.id !== configuredPost.id));
      
      // Remove from selected posts if it was there
      setSelectedPosts(prev => prev.filter(p => p.id !== configuredPost.id));
